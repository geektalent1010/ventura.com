...
class StoriesController extends Controller
{
    public function index()
    {
        ...
        $data['stories'] = Post::where('type', '=', 1)->where('is_active', '=', 1)->orderBy(
            'created_at',
            'desc'
        )->get();
    ...
    public function friendsStory()
    {
        ...
        $data['stories'] = Post::where('type', '=', 1)->whereIn('created_by', $friendIds)->where(
            'is_active',
            '=',
            1
    ...
    {
        $data['authUser'] = auth()->user();
        $data['stories'] = Post::where('type', '=', 1)->where('is_active', '=', 1)->where(
            'created_by',
            $data['authUser']->id
        )->orderBy('created_at', 'desc')->get();
