          ...
          <div class="member-link">
            <div class="member-avatar-wrp">
              <div class="member-avatar">
                @if ($post->user->profile->main_avatar_url)
                    <img src="{{ asset('uploads/'.$post->user->username.'/'.$post->user->profile->main_avatar_url.'?'.time()) }}">
                @else
                    <p class="first_letter">{{ $post->user->getMono() }}</p>
                @endif
              </div>
            </div>
            <div class="member-name">{{ $post->user->getFullname() }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="like-section">
        ...
        @if ($post->created_by === $authUser->id)
          <div class="option-icon-section">
            <a class="option-icon-btn" href="{{ route('events.mine.edit', [ 'id' => $post->id ]) }}">
